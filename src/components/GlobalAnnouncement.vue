...
<template>
  <div v-if="show" class="bg-white text-[#e4007f] shadow-sm relative z-[100] border-b border-pink-100">
    <div class="container mx-auto px-4 py-3 flex items-start gap-4">
      ...
      <div class="flex-1 text-sm md:text-base leading-relaxed">
        ...
        <p class="opacity-90 text-gray-600">
            ...
            请 <button @click="goHome" class="underline hover:text-[#e4007f] font-bold text-gray-800">返回首页</button> 重新创建。
        </p>
        <p class="text-xs mt-1 text-gray-400">此次升级重构了底层数据结构，旧版数据无法保留，十分抱歉！(Test phase templates deprecated)</p>
      </div>
      <button 
        @click="close"
        class="shrink-0 p-1 hover:bg-gray-100 rounded-full transition-colors text-gray-400 hover:text-[#e4007f]"
        title="不再显示"
      >
